...
import { Skeleton } from "@repo/ds/skeleton";
...
export default function Page(props: { searchParams: PaginationType }) {
			...
			<main className="w-full flex flex-col justify-between relative">
				...
				<Suspense fallback={<LoadingState />}>
					{createElement(async function Render() {
						const [[data, pagination]] = await parallel(profileService.index(props.searchParams));
						...
						return (
							<>
								<div className="flex mb-auto flex-wrap justify-between gap-4 my-4">
									...
									{data.length > 0 &&
										Array.from(new Array((pagination.limit - data.length) % 4)).map((x) => (
	...
	return (
		<div className="flex mb-auto flex-wrap justify-between gap-4 my-4">
			{Array.from(new Array(8)).map((_, i) => (
				<Skeleton key={i} className="max-w-[47%] sm:max-w-[210px] w-full aspect-card" />
			))}
		</div>
	);
}
