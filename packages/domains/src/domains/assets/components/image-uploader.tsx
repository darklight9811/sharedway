...
import { useUpdate } from "@repo/ds/hooks/use-update";
...
export default function ImageUploader(props: Props) {
	...
	const [files, setFiles] = useControllableState<Filesque[]>({
		prop: props.value,
	...
	useUpdate(() => {
		if (props.value && JSON.stringify(props.value) !== JSON.stringify(files)) setFiles(props.value);
	}, [props.value]);
			...
			<div className="flex flex-wrap mt-4 gap-2">
				...
				{(!props.max || files.length < props.max) && (
					<button
						onClick={() => open()}
						type="button"
						className=" pb-2 pr-2 relative aspect-square w-full text-foreground max-w-24 rounded-xl border-foreground border-4 border-dashed flex justify-center items-center"
					>
						<Camera size="52" />
